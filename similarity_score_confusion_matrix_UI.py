    ...
    for i, frame in enumerate(pred_data):
        ...
        for pred in pred_faces:
            ...
            for idx, gt in enumerate(gt_faces):
                ...
                if iou_val >= 0.0 and iou_val > best_iou:
                    best_iou = iou_val
                    best_gt_idx = idx
...
pred_file = st.sidebar.file_uploader("Upload Prediction JSON", type="json")
gt_file = st.sidebar.file_uploader("Upload Ground Truth JSON", type="json")
...
if pred_file and gt_file:
    ...
    st.info("Please upload both prediction and ground truth JSON files to begin.")
