    ...
    for i, frame in enumerate(pred_data):
        ...
        for pred in pred_faces:
            ...
            for idx, gt in enumerate(gt_faces):
                ...
                if iou_val >= 0.5 and idx not in used_gt:
                    used_gt.add(idx)
                    matched = True
...
with col2:
    ...
    confusion_matrix = pd.crosstab(df["Ground Truth"], df["Predicted"], rownames=["GT"], colnames=["Predicted"], dropna=False)
