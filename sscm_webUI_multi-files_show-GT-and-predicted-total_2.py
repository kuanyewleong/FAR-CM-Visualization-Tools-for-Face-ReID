...
if pred_files and gt_files:
    ...
    with col2:
        ...
        # Total based on predicted boxes (sum of all predictions)
        total_predicted = confusion_matrix.sum().sum()

        # Total based on ground truth boxes (sum across rows)
        total_gt = confusion_matrix.sum(axis=1).sum()

        # Correct predictions = sum of diagonal elements where GT == Pred
        ...
        ax2.set_title(
            f"GT vs Predicted | Acc (Pred): {accuracy_pred:.2%} "
            f"| Acc (GT): {accuracy_gt:.2%}\n"
